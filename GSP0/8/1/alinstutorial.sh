...
mkdir alinstutorial && cd alinstutorial
...
exports.GCFunction = (req, res) => {
    let message = req.query.message || req.body.message || 'Subscribe to Alins Tutorial';
...
DATA=$(printf 'Subscribe to Alins Tutorial' | base64) && gcloud functions call GCFunction --region=$REGION --data '{"data":"'$DATA'"}'
